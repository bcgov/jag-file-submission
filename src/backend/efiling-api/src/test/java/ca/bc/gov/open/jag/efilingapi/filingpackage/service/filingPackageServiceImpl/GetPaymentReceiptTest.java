...
public class GetPaymentReceiptTest {
    ...
    public void withValidRequestReturnFilingPackage() {
        ...
        Mockito.when(efilingReviewServiceMock.getPaymentReceipt(ArgumentMatchers.any())).thenReturn(Optional.of(BYTES));

        Optional<Resource> result = sut.getPaymentReceipt(BigDecimal.ONE);
    ...
    public void withValidRequestButMissingPackageReturnEmpty() {
        ...
        Mockito.when(efilingReviewServiceMock.getPaymentReceipt(ArgumentMatchers.any())).thenReturn(Optional.empty());

        Optional<Resource> result = sut.getPaymentReceipt(BigDecimal.TEN);

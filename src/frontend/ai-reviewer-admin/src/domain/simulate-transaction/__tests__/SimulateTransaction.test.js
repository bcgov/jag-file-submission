...
describe("Simulate Transaction Test Suite", () => {
  ...
  const file = new File(["content"], "fakeFile.pdf", {type: "application/pdf"});
  ...
  test("Get Document Types - success", async () => {
    ...
    await waitFor(() => {getByText("fakeFile.pdf")})
  ...
  test("Get Document Types - failure", async () => {
    ...
    dispatchEvt(dropzone, "drop", data);
    await waitFor(() => {getByText("Error: Could not load configurations.")})

    expect(getByText("Error: Could not load configurations.")).toBeInTheDocument();
  })
  ...
  test("Submit file for extraction - success", async () => {
    ...
    const { container, getByText, getByTestId } = render(<SimulateTransaction />);
    const dropzone = container.querySelector('[data-testid="dropdownzone"]');
  ...
  test("Submit file for extraction - failure", async () => {
    ...
    const { container, getByText, getByTestId, queryByText } = render(<SimulateTransaction />);
    const dropzone = container.querySelector('[data-testid="dropdownzone"]');
    ...
    expect(queryByText("Document Processed Successfuly")).not.toBeInTheDocument();
  })

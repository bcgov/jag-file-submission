Feature: New CSO accounts can be created if a BCeID profile is not associated with a CSO account
 ...
 # @cso
  Scenario: Verify a CSO account cannot be created for requests made with incorrect path
    Given POST http request is made to "CREATE_CSO_ACCOUNT_API" with incorrect path value
  ...
  #@cso
  Scenario: Verify a CSO account cannot be created for requests made without the request body
    Given POST http request is made to "CREATE_CSO_ACCOUNT_API" without request body
    When status is 415 and content type is verified
